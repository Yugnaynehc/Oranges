...
PUBLIC void init_screen(TTY *p_tty)
{
    ...
    int v_mem_size = V_MEM_SIZE >> 1; /* total size of video memory(in word) */
    ...
    /* cursor is in original address defaultly */
...
PUBLIC void select_console(int nr_console)
{
    ...
    CONSOLE current_console = console_table[nr_console];
    
    set_cursor(current_console.cursor);
    set_video_start_addr(current_console.current_start_addr);
}
...
PUBLIC void scroll_screen(CONSOLE *p_con, int direction)
{
    if (direction == SCR_UP) {
        ...
        if (p_con->current_start_addr + SCREEN_SIZE + SCREEN_WIDTH <
